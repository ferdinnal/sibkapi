...
class Mjadwal_pelajaran extends CI_Model
{
    ...
    public function find_siswa_detail_2($hari, $result_type, $option = null)
    {
        ...
        $this->db->select($this->table_mata_pelajaran.".matpel");
        $this->db->select($this->table_pengguna.".fullname as nama_guru");
        $this->db->select($this->table_ruangan.".name as nama_ruangan");
        ...
        $this->db->join($this->table_pengguna, $this->table_mata_pelajaran . '.userid = ' . $this->table_pengguna . '.userid');
        $this->db->join($this->table_detail_siswa, $this->table_kelas . '.id_kelas = ' . $this->table_detail_siswa . '.id_kelas');
        $this->db->join($this->table_ruangan, $this->table . '.ruangid = ' . $this->table_ruangan . '.ruangid');
        $this->db->where($this->table . '.hari', $hari);
        $this->db->order_by($this->table.".mulai", 'ASC');
